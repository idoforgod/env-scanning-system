...
def fetch_transcript(video_id: str, preferred_language: str = "ko") -> dict:
    ...
    """
    result = {"available": False, "language": None, "text": None, "segments": None, "error": None}
        ...
        if not transcript:
            try:
                for t in transcript_list:
                    transcript = t
                    break
            except Exception:  # noqa: S110 - 단순 fallback 로직
                pass
...
def main():
    ...
    parser.add_argument("--include-segments", action="store_true", help="타임스탬프가 포함된 세그먼트 정보 포함")
