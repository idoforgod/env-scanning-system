...
def collect_channel_videos(
    ...
    """단일 채널의 영상을 수집하고 저장"""
    result = {"channel": channel_handle, "channel_id": channel_id, "new_videos": [], "skipped": 0, "errors": []}
    ...
    for video in videos:
        ...
        try:
            ...
            result["errors"].append({"video_id": video_id, "error": str(e)})
...
def main():
    ...
    if not args.all and not args.channel_id and not args.channel_handle:
        print(json.dumps({"error": "--all, --channel-id, --channel-handle 중 하나를 지정해야 합니다."}))
        sys.exit(1)
    ...
    if args.all:
        ...
        if not channels:
            print(
                json.dumps(
                    {"error": "등록된 채널이 없습니다.", "help": "먼저 register_channel.py로 채널을 등록해주세요."}
                )
            )
            sys.exit(1)

        for ch in channels:
            channels_to_process.append({"id": ch.get("id"), "handle": ch.get("handle", "")})
    else:
        # 단일 채널 처리
        ...
        if not channel_id and channel_handle:
            ...
            if not channel_id:
                print(json.dumps({"error": f"채널을 찾을 수 없습니다: {channel_handle}"}))
                sys.exit(1)

        channels_to_process.append({"id": channel_id, "handle": channel_handle})
    ...
    for channel in channels_to_process:
        try:
            ...
            all_results["errors"].append({"channel": channel.get("handle") or channel.get("id"), "error": str(e)})
