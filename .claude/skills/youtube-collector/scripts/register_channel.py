...
def main():
    ...
    if not args.channel_handle and not args.channel_url and not args.channel_id:
        print(json.dumps({"error": "--channel-handle, --channel-url, --channel-id 중 하나를 지정해야 합니다."}))
        sys.exit(1)
    ...
    if args.channel_url:
        ...
        if not extracted:
            print(json.dumps({"error": "URL에서 채널 정보를 추출할 수 없습니다.", "url": args.channel_url}))
            sys.exit(1)
    ...
    if not channel_info:
        print(json.dumps({"error": f"채널 정보를 가져올 수 없습니다: {channel_id}"}))
        sys.exit(1)
            ...
            json.dumps(
                {
                    ...
                    "channel": {"id": channel_id, "handle": channel_handle, "name": channel_info["name"]},
                }
            )
        )
        sys.exit(0)
