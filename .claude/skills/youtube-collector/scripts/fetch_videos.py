...
try:
    ...
    print(json.dumps({"error": "pyyaml이 설치되어 있지 않습니다.", "install": "pip install pyyaml"}))
    sys.exit(1)
...
def get_channel_id_from_handle(youtube, handle: str) -> str:
    ...
    try:
        response = youtube.search().list(part="snippet", q=f"@{handle}", type="channel", maxResults=1).execute()
...
def fetch_videos(youtube, channel_id: str, max_results: int = 10) -> list:
    ...
    try:
        ...
        response = (
            ...
            .list(part="snippet,contentDetails", playlistId=uploads_playlist_id, maxResults=min(max_results, 50))
            .execute()
        )
        ...
        # 영상 상세 정보 가져오기
        videos_response = youtube.videos().list(part="snippet,contentDetails", id=",".join(video_ids)).execute()
        ...
        print(json.dumps({"error": f"YouTube API 오류: {e.resp.status}", "message": str(e)}), file=sys.stderr)
        return videos
...
def main():
    ...
    if not args.channel_id and not args.channel_handle:
        print(json.dumps({"error": "--channel-id 또는 --channel-handle 중 하나를 지정해야 합니다."}))
        sys.exit(1)
            ...
            json.dumps(
                {
                    ...
                    "help": "다음 명령으로 API 키를 설정하세요: python3 setup_api_key.py",
    ...
    try:
        ...
        print(json.dumps({"error": "YouTube API 초기화 실패", "message": str(e)}))
        sys.exit(1)
    ...
    if not channel_id and args.channel_handle:
        ...
        if not channel_id:
            print(json.dumps({"error": f"채널을 찾을 수 없습니다: {args.channel_handle}"}))
            sys.exit(1)
